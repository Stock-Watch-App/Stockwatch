    ...
    @if(Auth::check() && Auth::user()->hasVerifiedEmail() && Auth::user()->isPlaying())
        ...
        <div id="market-banner" class="info flash__message">
            @if($season->status === 'open')
                <span>The market will be closing at {{ $season->closes_at->format('h:i a') }} ET @if(((new DateTime())->setTimezone(new DateTimeZone('America/New_York'))->format('N')) === '4') today @else on Thursday @endif.</span>
